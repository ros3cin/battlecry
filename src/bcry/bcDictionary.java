...
class bcDictionary {
    ...
    private java.util.Map database;

    private bcVoice voice;

    public bcDictionary(String dictfile, bcVoice v) {
        database = new org.eclipse.collections.impl.map.mutable.ConcurrentHashMap();
        voice = v;
        loadDict(dictfile);
    }
    ...
    public String getPhonemes(String word) {
        ...
        String result = "";
        if (word.indexOf(" ") != -1) {
            // if String contains more than one word, process them one by one
